...
function plotRes(problem_setting, states_his, optStates)
	...
	h = plot!(h, circleShape(problem_setting["goal_pt"][1], problem_setting["goal_pt"][2], 7.2), seriestype = [:shape,], ;w = 0.5, c=:green, linecolor = :green,framestyle = :box, legend = false,xlims = (-10, 110), ylims = (-10, 10), fillalpha = 1.0, xlabel = "X (m)", ylabel = "Y (m)", xtickfontsize=14,ytickfontsize=14,xguidefontsize=14,yguidefontsize=14)

    h = plot!(h, optStates[:,1], optStates[:,2],lw = 2, aspect_ratio=:equal, lc=:red, legend=false)
    h = plot!(states_his[2,:], states_his[3,:], lw = 2, aspect_ratio=:equal, lc=:green, xlims = (-10, 110), ylims = (-10, 10))
