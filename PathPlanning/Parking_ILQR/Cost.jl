...
function StageCost(states, control)
    ...
    J       = 0.01*ax^2 + 0.01*δ^2 + 10*y^2 + 0.5*x^2 + 100*ψ^2 +0.01*ux^2 + δ_bound_cost + ax_bound_cost
...
function TerminalCost(states, control)
    ...
    J = 10*((x - terminal_states[1])^2 + (y - terminal_states[2])^2 + 0.1*(ux - terminal_states[3])^2 + 1*(ψ - terminal_states[4])^2)
    # J       = 0.1*(x - 100) ^ 2
    return J
end
