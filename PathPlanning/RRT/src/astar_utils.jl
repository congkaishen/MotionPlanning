...
function FindNewNode(astar::AstarSearcher, current_node::AstarNode)
    ...
    for direction in directions
        ...
        idxs_newpoint, ~ = knn(terrain_kdtree, TransferCoordinate(astar, new_position), 5, true)
        height = astar.s.terrain_info[3, idxs_newpoint[1]]
        ...
        if checkPositionValidity(astar, new_position)
            ...
            temp_g =current_node.g + sqrt((new_position[1] - current_node.position[1])^2*astar.s.x_factor^2 + (new_position[2] - current_node.position[2])^2*astar.s.y_factor^2 + (height - current_node.height)^2)
